...
class Ruta
{
    constructor(_origen, _destino, _terminado){
        this.origen = _origen;
    ...
    borrarDelArray(actual){
        for(let i = this.openSet.length - 1; i>=0; i--){
            if(this.openSet[i] == actual){
                this.openSet.splice(i,1);
            }
        }
    }
    algoritmo(){
        let ganador, actual, temporal, V,v,tempG;
            ...
            if(this.openSet.length > 0){
                ...
                for(let i = 0; i < this.openSet.length; i++){
                    ...
                    if(this.openSet[i].f < this.openSet[ganador].f){
                        //ACTUALIZAR EL INFICE DEL MEJOR
                        ganador = i;
                    }
                }
                //ANALIZAR LA MEJOR CASILLA
                actual = this.openSet[ganador];
...
function inicio()
{
    ...
    //RUTA
    ruta = new Ruta(origen,destino,false);
